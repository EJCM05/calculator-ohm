<template>
  <div class="cover-container d-flex w-100 h-100 p-3 mx-auto flex-column text-center">
    ...
    <main class="container-fluid">
      <button @click="toggleComponent" class="btn btn-outline-primary mt-4">{{ buttonText }}</button>
      <ResistorCalculator v-if="showCalculator"></ResistorCalculator>
...
import ResistorCalculator from '@/components/Calculator.vue';
import SmdResistorCalculator from './components/Calculator-smd.vue';
...
const buttonText = computed(() => {
  return showCalculator.value ? 'Cambiar a Calculadora SMD' : 'Cambiar a Calculadora de Color';
});
</script>
...
.color-ohm {
  font-size: 2.5rem;
  color: var(--color-accent-ohm) !important;
}
